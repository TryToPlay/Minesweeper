        ...
        if not gamedone:
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_f:
                    for block in block_list:     
                        if block.hitbox.collidepoint(pygame.mouse.get_pos()) and block.hidden:
                            block.flag()
                            if block.flagged:
                ...
                if event.key == pygame.K_d:
                    for block in block_list:
                        if block.hitbox.collidepoint(pygame.mouse.get_pos()) and not block.flagged:
                    ...
                    for number in number_list:
                        if number.hitbox.collidepoint(pygame.mouse.get_pos()) and not number.hidden:
                            if len([block for block in block_list if block.hitbox.center in number.neighbours and block.flagged]) == number.value:
                                for block in block_list:
                                    if block.hitbox.center in number.neighbours and not block.flagged:
                                        pygame.mixer.Channel(2).play(dig_sfx)
                                        block.dig(block_list)
                                        if block in bomb_list:
                                            pygame.mixer.Channel(1).play(explosion_sfx)
